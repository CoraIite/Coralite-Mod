...
namespace Coralite.Content.Bosses.ModReinforce.PurpleVolt
{
    ...
    /// ai0传入闪电时间，ai1传入主人
    /// </summary>
    public class PurpleDischargingBurst : BaseZacurrentProj
    {
        ...
        public override void SetDefaults()
        {
            ...
            Projectile.width = Projectile.height = 530;
        }
        ...
        public override void Initialize()
        {
            ...
            {
                if (i == 0)
                    circles[i] = new ThunderTrail(thunderTex, ThunderWidthFunc2, ThunderColorFunc2_Pink, GetAlpha);
                else
            ...
            {
                if (i < 2)
                    outers[i] = new ThunderTrail(thunderTex, ThunderWidthFunc_Sin, ThunderColorFunc2_Pink, GetAlpha);
                else
        ...
        public void UpdateTrails()
        {
            if (Timer > 10 && Timer % 4 == 0)
                foreach (var outer in outers)
                {
                    ...
                    {
                        Vector2[] vec = new Vector2[6];
                        float length = Main.rand.NextFloat(2, 24);
                        Vector2 basePos = Projectile.Center + (Helper.NextVec2Dir() * length);
                        Vector2 dir = (basePos - Projectile.Center).SafeNormalize(Vector2.Zero);
                        vec[0] = basePos;
            ...
            if (Timer > 10 && Timer % 6 == 0)
                foreach (var circle in circles)
                {
                    ...
                    if (canDraw)//从内向外散发出去的闪电
                    {
                        int trailPointCount = Main.rand.Next(15, 26);
                        Vector2[] vec = new Vector2[trailPointCount];
        ...
        public virtual void SpawnDusts()
        {
            if (Timer % 2 == 0)
            {
                PRTLoader.NewParticle(Projectile.Center + Main.rand.NextVector2Circular(Projectile.width / 5, Projectile.width / 5),
                    Vector2.Zero, CoraliteContent.ParticleType<BigFog>(), ZacurrentDragon.ZacurrentPink * Main.rand.NextFloat(0.5f, 0.8f),
                    Main.rand.NextFloat(2f, 2.5f));
                ElectricParticle_PurpleFollow.Spawn(Projectile.Center, Helper.NextVec2Dir(Projectile.width / 3, Projectile.width * 0.56f), () => Projectile.Center, Main.rand.NextFloat(0.7f, 1.1f));
            }
            else
            {
                Dust.NewDustPerfect(Projectile.Center + Main.rand.NextVector2Circular(Projectile.width / 3, Projectile.width / 3)
                    , ModContent.DustType<LightningShineBall>(), Vector2.Zero, newColor: ZacurrentDragon.ZacurrentPurpleAlpha, Scale: Main.rand.NextFloat(0.3f, 0.5f));
            }
        }
