        ...
        public void GenClearGemstoneMaze(GenerationProgress progress, GameConfiguration configuration)
        {
            ...
            for (int i = 0; i < 1000; i++)
            {
                try
                {
                    ...
                    //每隔一段选取一个点并检测是否有邪恶地形
                    int junglePos = Main.rand.Next(GenVars.jungleOriginX - 200, GenVars.jungleOriginX + 200);//(GenVars.jungleMaxX + GenVars.jungleMinX) / 2;
                    ...
                    int whichOne = WorldGen.genRand.Next(1);
                    ...
                    Texture2D clearTex = ModContent.Request<Texture2D>(AssetDirectory.WorldGen + "CoreKeeper/ClearGemstoneMazeClear" + whichOne.ToString(), AssetRequestMode.ImmediateLoad).Value;
