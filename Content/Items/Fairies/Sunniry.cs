...
namespace Coralite.Content.Items.Fairies
{
    ...
    public class SunniryProj : BaseFairyProjectile
    {
            ...
            switch (State)
            {
                ...
                case AIStates.Backing:
                    if (Main.rand.NextBool(3))
                        Helper.SpawnTrailDust(center, velocity, Projectile.width, DustID.GoldCoin, Main.rand.NextFloat(0.1f, 0.5f), 200);
                    break;
                case AIStates.Skill:
                default:
                    Helper.SpawnTrailDust(center, velocity, Projectile.width, DustID.GoldCoin, Main.rand.NextFloat(0.1f, 0.5f), 200);
        ...
        {
            return new Vector2(0, MathF.Sin(Timer * 0.2f + Projectile.key * MathHelper.TwoPi / 6) * 3);
        }
