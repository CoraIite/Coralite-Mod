...
{
    public class ThunderveinDragonBossBag : ModItem
    {
        ...
        public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
        {
            ...
            for (float i = 0f; i < 1f; i += 0.25f)
            {
                ...
                spriteBatch.Draw(texture, drawPos + (new Vector2(0f, 8f).RotatedBy(radians) * time), frame, new Color(219, 114, 22, 50), rotation, frameOrigin, scale, SpriteEffects.None, 0);
            }

            for (float i = 0f; i < 1f; i += 0.34f)
            {
                float radians = (i + timer) * MathHelper.TwoPi;
                spriteBatch.Draw(texture, drawPos + (new Vector2(0f, 4f).RotatedBy(radians) * time), frame, new Color(255, 202, 101, 77), rotation, frameOrigin, scale, SpriteEffects.None, 0);
            }
