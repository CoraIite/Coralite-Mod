    ...
    public class MagConnectStaff : ModItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            switch (mode)
            {
                ...
                case 0://连接模式
                    {
                        ...
                        {
                            if (MagikeHelper.TryGetEntity(pos.X, pos.Y, out MagikeSender magS))    //找到了
                            {
                                sender = magS;
                ...
                case 1://查看模式
                    {
                        if (MagikeHelper.TryGetEntity(pos.X, pos.Y, out MagikeSender magS))  //找到了，显示所有的连接
                            magS.ShowConnection();
                        else
                            CombatText.NewText(rectangle, Coralite.Instance.MagicCrystalPink, "未找到发送器");
                    }
                ...
                case 2://断连模式
                    {
                        if (MagikeHelper.TryGetEntity(pos.X, pos.Y, out MagikeSender magS))  //找到了，断开所有的连接
                        {
                            magS.DisconnectAll();
