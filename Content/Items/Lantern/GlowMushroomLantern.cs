...
namespace Coralite.Content.Items.Lantern
{
    public class GlowMushroomLantern : ModItem
    {
        ...
        {
            Vector2 pos = player.Center + new Vector2(player.direction * 10, -4);
    ...
    public class GlowMushroomLanternFire : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextBool())
                Projectile.SpawnTrailDust(ModContent.DustType<PixelPoint>(), Main.rand.NextFloat(0, 0.4f), newColor: new Color(23, 231, 155)
            ...
            if (Projectile.IsOwnedByLocalPlayer())
            {
                ...
                for (int i = -1; i < 2; i++)
                {
                    Projectile.NewProjectileFromThis<GlowMushroomLanternGrow>(Projectile.Center
                        , -Vector2.UnitY.RotateByRandom(i * 0.7f - 0.35f, i + 0.7f + 0.35f) * Main.rand.NextFloat(3,7), damage, Projectile.knockBack);
                }
            }
    ...
    public class GlowMushroomLanternGrow : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Timer < 15)
            {
                ...
                angle = angle.AngleLerp(-MathHelper.PiOver2, 0.07f);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Color c = Main.rand.NextFromList(new Color(25,142,196), new Color(20, 78, 129));
                PRTLoader.NewParticle<Fog>(Projectile.Center + Main.rand.NextVector2Circular(12, 12)
                    , Helper.NextVec2Dir(0.5f, 1f), c, Main.rand.NextFloat(0.2f, 0.5f));
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (Projectile.oldPos.Length==14)
            {
                for (int i = 0; i < 14; i++)
                {
                    ...
                    Vector2 Top = Center + (normal * 1.5f);
                    Vector2 Bottom = Center - (normal * 1.5f);

                    var color = Color.Lerp(new Color(23, 231, 255), new Color(20, 78, 129), factor) * factor;
