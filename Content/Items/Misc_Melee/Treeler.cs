...
namespace Coralite.Content.Items.Misc_Melee
{
    public class Treeler : ModItem
    {
        ...
        {
            Item.CloneDefaults(ItemID.Seedler);
            Item.useStyle = ItemUseStyleID.Rapier; // Makes the player do the proper arm motion
            Item.noUseGraphic = true; // The sword is actually a "projectile", so the item should not be visible when used
            Item.noMelee = true; // The projectile will do the damage and not the item

            Item.shootSpeed = 2f;
            Item.shoot = ModContent.ProjectileType<TreelerProj>(); // The projectile is what makes a shortsword work
        }

        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            Projectile.NewProjectile(source, player.MountedCenter
                , velocity.SafeNormalize(Vector2.Zero) * 6f, ProjectileID.SeedlerNut
        ...
        public void Transform()
        {
            if (!CoraliteWorld.coralCatWorld)
            {
                ...
                Item.SetDefaults(ItemID.Seedler);
            }
        }
    }

    public class TreelerProj : ModProjectile
    {
        public override string Texture => AssetDirectory.Misc_Melee + "Treeler";
