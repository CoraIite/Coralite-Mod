...
{
    public class DiamondFlower : BaseGemWeapon
    {
        public override void SetDefs()
        {
            ...
            Item.shoot = ModContent.ProjectileType<DiamondFlowerProj>();
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            else
            {
                foreach (var proj in Main.projectile.Where(p => p.active && p.owner == player.whoAmI && p.type == type))
                    (proj.ModProjectile as DiamondFlowerProj).StartAttack();
            }
        ...
        public override void DrawGemName(DrawableTooltipLine line)
        {
            DrawGemNameNormally(line, effect =>
            {
                ...
                effect.Parameters["highlightC"].SetValue(DiamondProj.highlightC.ToVector4());
                effect.Parameters["brightC"].SetValue(DiamondProj.brightC.ToVector4());
                effect.Parameters["darkC"].SetValue(DiamondProj.darkC.ToVector4());
            }, 0.1f);
        }

        public override void SpawnParticle(DrawableTooltipLine line)
        {
            SpawnParticleOnTooltipNormaly(line, DiamondProj.brightC, DiamondProj.highlightC);
        }
    ...
    public class DiamondFlowerProj : BaseGemWeaponProj<DiamondFlower>, IDrawAdditive
    {
        public override string Texture => AssetDirectory.LandOfTheLustrousSeriesItems + "DiamondFlower";
        ...
        public override void BeforeMove()
        {
            ...
            if (AttackTime < 1 && Main.rand.NextBool(6))
            {
                ...
                {
                    0 => new Color(218, 185, 210, 100),
                    1 => new Color(99, 99, 149, 100),
                ...
                Dust d = Dust.NewDustPerfect(pos, DustID.WhiteTorch, Helper.NextVec2Dir(1f, 3f));
                d.noGravity = true;
            }
            ...
            if ((int)Main.timeForVisualEffects % 20 == 0 && Main.rand.NextBool(2))
            {
                ...
                Color c2 = Main.rand.NextFromList(Color.White, DiamondProj.brightC, DiamondProj.darkC);
            ...
            if (AttackTime > 0)
            {
                ...
                else
                {
                    ...
                    if (AttackTime == 1)
                    {
                        ...
                        for (int i = 0; i < 8; i++)
                        {
                            Dust d = Dust.NewDustPerfect(Projectile.Center, DustID.WhiteTorch, Helper.NextVec2Dir(2, 4), Scale: Main.rand.NextFloat(1f, 1.5f));
                            d.noGravity = true;
                        }

                        for (int i = 0; i < 5; i++)
                            DiamondProj.SpawnTriangleParticle(Projectile.Center + dir.RotateByRandom(-0.5f, 0.5f) * Main.rand.NextFloat(6, 12), dir * Main.rand.NextFloat(1f, 3f));
                    }
                }
        ...
        {
            Projectile.DrawShadowTrails(DiamondProj.brightC, 0.3f, 0.3f / 4, 0, 4, 1, -0.4f, -1);
            Projectile.QuickDraw(lightColor, -0.4f);
            return false;
        }
    ...
    public class DiamondProj : ModProjectile, IDrawNonPremultiplied
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextBool())
            {
                ...
                Projectile.SpawnTrailDust(8f, DustID.WhiteTorch, Main.rand.NextFloat(0.2f, 0.4f));
            }
        }
    ...
    public class DiamondExplosion : ModProjectile
    {
        ...
        public List<DiamondExplosionData> datas = new();
        public Vector2 rand = Main.rand.NextVector2CircularEdge(64, 64);

        public class DiamondExplosionData
        {
            ...
            public DiamondExplosionData(Vector2 center, float width, float height, float scale)
            {
                this.width = width;
                this.height = height;
                this.scale = scale;
            ...
            public void Update()
            {
                ...
                if (timer % 6 == 0)
                    DiamondProj.SpawnTriangleParticle(Center + Main.rand.NextVector2Circular(12, 12), velocity * Main.rand.NextFloat(0.2f, 0.4f));
                if (Main.rand.NextBool(15))
                {
                    Dust dust = Dust.NewDustPerfect(Center + Main.rand.NextVector2Circular(6, 6), DustID.WhiteTorch, Vector2.Zero);
                    dust.noGravity = true;
            ...
            if (Timer < 20 && Timer % 2 == 0)
            {
                ...
                if (owner.CheckMana(1, true))
                {
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        var data = new DiamondExplosionData(Projectile.Center + dir * length, 30, 30, 0.5f + factor * 0.4f)
                        {
                            rotation = rot,
                        };
