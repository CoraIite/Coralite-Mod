...
namespace Coralite.Core.Prefabs.Tiles
{
    ...
    /// <summary>
    /// 请使用BotanicalHelper.UpdatePlant以更新物块
    /// 请使用BotanicalHelper.PlacePlant以放置在世界中
    /// 请使用BotanicalHelper.KillPlant以Kill该位置的Entity
    /// </summary>
    public abstract class BasePlantTile<T> : ModTile where T : BasePlantTileEntity
    {
        private readonly string _Texture;
        private readonly bool PathHasName;
        private readonly short FrameWidth;
        private readonly int FrameCount;
        private readonly int SeedType;
        private readonly int PlantType;
        private readonly int RarePlantType;

        public BasePlantTile( string texture, short frameWith, int frameCount, int seedType = 0, int plantType = 0, int rarePlantType = 0, bool pathHasName = false)
        {
            _Texture = texture;
            FrameWidth = frameWith;
        ...
        public override bool Drop(int i, int j)
        {
            ...
            {
                int id = Item.NewItem(null, worldPosition, RarePlantType, rarePlantStack);
            ...
            {
                int id = Item.NewItem(null, worldPosition, PlantType, plantStack);
                if (id > ItemID.Count)
                    if (BotanicalHelper.TryGetTileEntityAs(i, j, out T entity))
                    {
            ...
            {
                int id= Item.NewItem(null, worldPosition, SeedType, seedStack);
                if (BotanicalHelper.TryGetTileEntityAs(i, j, out T entity))
                {
                    Item plantItem = Main.item[id];
                    BotanicalItem botanicalItem = plantItem.GetBotanicalItem();
